這份 `index.html` 修正得非常到位，標題統一把「地鐵」調整為更貼合香港日常語境的「港鐵」，無障礙屬性（`aria-pressed` 與 `aria-expanded`）齊全，結構清晰。

以下為你量身打造的正式開源雙語 `README.md`，完整結合了**專業調解心理學（ADR 原則）**、**高標準前端工程規範**與**開源共建指南**。
